                    ...
                    if (slaveLabel == "Windows") {
                        try {
                            ...
                            script.withEnv(["CONAN_TEST_FOLDER=${workdir}"]) {
                                script.bat(script: "python python_runner/runner.py ${this.testModule} ${pyver} ${sourcedir} \"${workdir}\" ${numcores} ${flavor_cmd} ${eTags}", returnStatus: true)
                            }
                        }
                        finally {
                        ...
                        try {
                            script.withEnv(['PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin']) {
                                script.sh(script: "python python_runner/runner.py ${this.testModule} ${pyver} ${sourcedir} ${workdir} ${numcores} ${flavor_cmd} ${eTags}", returnStatus: true)
                            }
                        }
                        finally {
                        ...
                        try {
                            ...
                            script.docker.image('conanio/conantests').inside() {
                                ...
                                script.sh(script: "su - conan -c \"python ${sourcedir}/python_runner/runner.py ${testModule} ${pyver} ${sourcedir} /tmp ${numcores} ${flavor_cmd} ${eTags}\"", returnStatus: true)
                            }
                        }
                        finally {
                            script.sh(script: "rm -rf ${sourcedir}")
